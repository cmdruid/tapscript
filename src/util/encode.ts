...
import type { DecodedData } from '@/types/index.js'
...
function bech32_decode (
  ...
  const data = Buff.raw(to_bytes(rest)).hex
...
export function decode_data (
  ...
  if (type === 'base58') {
    return Buff.b58chk(str).slice(1).hex
  } else if (type === 'bech32') {
...
export const Base58CHK = {
  encode : (data : Bytes)  => Buff.bytes(data).to_b58chk(),
  decode : (str  : string) => Buff.b58chk(str).slice(1)
}
