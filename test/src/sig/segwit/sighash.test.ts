...
export function sighash_vector_test(t :Test) {
  t.test('Testing segwit sighash vectors.', t => {
    ...
    for (const vector of sign_vectors) {
      ...
      try {
        const hash = Signer.segwit.hash_tx(txdata, { txindex: index, ...config })
      ...
      try {
        ...
        const sig = Signer.segwit.sign_tx(seckey, txcopy, { txindex: index, ...config })
        ...
        const signerVerify = Signer.segwit.verify_tx(txcopy, { txindex: index, ...config })
